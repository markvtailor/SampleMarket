...
internal class FilterSalesUseCase @Inject constructor(
    ...
    operator fun invoke(filterOption: FilterOption): Flow<List<BestSale>> = flow {
        ...
        val filtered: List<BestSale> = if (filterOption.brand.isBlank() && filterOption.maxPrice == 0L && filterOption.maxSize == 0.0) {
            sales
            ...
            val filteredByBrand = filterByBrand(sales, filterOption.brand)
            //filterBySize
            val filteredByPrice = filterByPrice(filteredByBrand, filterOption.maxPrice, filterOption.minPrice)
            filteredByPrice
        }
    ...
    private fun filterByPrice(salesList: List<BestSale>, maxPrice: Long, minPrice: Long): List<BestSale> {
